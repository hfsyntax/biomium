...
else { console.log(`No server`), proceed = false }
...
//let chatKey = localStorage.getItem('chat-key') || 90
...
if (proceed) {
    const init = () => {
        unsafeWindow.setTimeout(() => {
            ...
            document.querySelector("head > link:nth-child(7)").href = "https://drive.google.com/uc?export=download&id=1dyN1LKU0yTtv1lBLdf1Ew2cIdrZrji08"
            ...
            /*let chatKey_input = document.getElementById('chatKey-input')
            let chatbox = document.getElementById('chatbox')
            let chatNick = document.getElementById('name-input')
            let chat = document.getElementById("chat-container");
            chatbox.style.display = "none";
            chatKey_input.value = localStorage.getItem('chat-key-value') || keyboardMap[chatKey]
            chatKey_input.onchange = function(){
                localStorage.setItem('chat-key', chatKey)
                localStorage.setItem('chat-key-value', chatKey_input.value)

            }
            unsafeWindow.addEventListener("keyup", chatKeys, false);
            function chatKeys(key) {
                if (isInputting == false && chatKey != splitKey && chatKey != doubleKey && chatKey != maxKey && chatKey != respawnKey && chatKey != navKey) {
                    if (key.keyCode == chatKey ){
            ...
            chatKey_input.onkeyup = function(key){
                isInputting = true
                chatKey = key.keyCode
                chatKey_input.value = keyboardMap[chatKey]
                setTimeout(() => { isInputting = false }, 1000)
            }
            ...
            function sendMessage(key) {
                if (key.keyCode === 13) {
                    if (chatbox.value !== "") {
                        if(chatNick.value === ""){
                            chatNick.value = "noname"
                        }
                        const child = document.createElement("div");
                        child.textContent = `[${chatNick.value}] ${chatbox.value}`
                        child.className = "messages"
                        chat.appendChild(child);
                        chatbox.value = "";
                        chatbox.style.display = "none";
                        isInputting = false
                        socket.broadcast.emit('chatMsg', elemToString(child))
                        console.log('passed')
                        setTimeout(()=>{
                            child.remove()
                        },4000)
            ...
            nameInput.onkeydown = function(key){
                if(key.keyCode === 32){
                    nameInput.value = this.value + " "
                }
            }

            chatbox.onkeydown = function(key){
                isInputting = true
                if(key.keyCode === 32){
                    chatbox.value = this.value + " "
                }
            }
            */
